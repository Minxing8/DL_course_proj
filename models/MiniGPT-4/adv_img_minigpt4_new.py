...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_tag", default="adv_unknown", type=str, help="Tag for the dataset (e.g., car_adv)")
    ...
    parser.add_argument(
        "--options",
        nargs="+",
        help="Override some settings in the used config, the key-value pair in xxx=yyy format will be merged into config file (deprecated), use --cfg-options instead.",
        default=None
    )
    ...
    # Start timing for adversarial image generation
    ...
    avg_time = total_time / processed_samples if processed_samples > 0 else 0.0
    print(f"Adversarial attack completed. Processed {processed_samples} images.")
    print(f"Average time per image: {avg_time:.5f} seconds.")

    # Append timing info to the time log file
    with open(args.time_log, "a") as f:
        f.write(f"minigpt4_on_{args.dataset_tag}: {avg_time:.5f} seconds per image (adversarial)\n")
